...
SHEET_ID = "1jZwhiehWGGqVNucPIB7URzrhg_-vASpwFJtgg1ml5Mg"
...
def load_sheet_data():
    ...
    # row_number real da planilha (começando da linha 2, porque linha 1 é cabeçalho)
...
with f1:
    if meses_validos:
        selected_month = st.selectbox("Mês", ["Todos"] + meses_validos, index=1 if len(meses_validos) >= 1 else 0)
    else:
        selected_month = st.selectbox("Mês", ["Todos"], index=0)
